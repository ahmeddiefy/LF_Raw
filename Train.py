...
if __name__ == '__main__':
    ...
    if not USE_QUEUE_LOADING:
        ...
        q = tf.FIFOQueue(10000, [tf.float32, tf.float32], [[IMG_SIZE[0], IMG_SIZE[1], 1], [OUT_IMG_SIZE[0], OUT_IMG_SIZE[1], 1]])
    ...
    with tf.Session(config=config) as sess:
        ...
        if USE_QUEUE_LOADING:
            ...
            for epoch in range(0, MAX_EPOCH):
                ...
                saver.save(sess, "./checkpoints/LF_epoch_%03d.ckpt" % epoch ,global_step=global_step)
        else:
            for epoch in range(0, MAX_EPOCH):
